...
import { Dialog, Transition, Switch } from '@headlessui/react'; // Import Switch for toggle
import {
  XMarkIcon,
  PlusCircleIcon,
  PencilSquareIcon,
  ...
  CogIcon,
  UsersIcon, // For assign students button
  CheckIcon, // For selected students in list
  ChevronDownIcon, // For dropdowns
  ChevronUpIcon, // For dropdowns
} from '@heroicons/react/24/outline'; // Changed to 24/outline for consistency
import { useRouter } from 'next/navigation';

type Class = Database['public']['Tables']['classes']['Row'];
type Student = Database['public']['Tables']['students']['Row'];

// NEW TYPE: For students fetched specifically for the assignment modal
type StudentForAssignment = Pick<Student, 'id' | 'name' | 'roll_no'>;

type FeeType = Database['public']['Tables']['fee_types']['Row'] & {
  classes?: Partial<Class>[]; // Ensure 'classes' property holds Partial<Class>[]
};
type StudentFeeType = Database['public']['Tables']['student_fee_types']['Row'];

// NEW TYPE: For partial student_fee_types data fetched for assignment tracking
type AssignedStudentFeeTypeRef = Pick<StudentFeeType, 'id' | 'student_id'>; // Added 'id' as it's needed for deletion
...
export default function FeeTypeManagement() {
  ...
  const [createApplicableFromEnabled, setCreateApplicableFromEnabled] = useState(false); // NEW: Toggle for 'Applicable From'
  ...
  const [editApplicableFromEnabled, setEditApplicableFromEnabled] = useState(false); // NEW: Toggle for 'Applicable From'

  // Assign Students Modal States (NEW)
  ...
  const [studentsInSelectedClass, setStudentsInSelectedClass] = useState<StudentForAssignment[]>([]); // CHANGED TYPE HERE
  ...
  const [assignedStudentFeeTypes, setAssignedStudentFeeTypes] = useState<AssignedStudentFeeTypeRef[]>([]); // CHANGED TYPE HERE
  
  // NEW: Filtered list of classes for the Assign Students modal
  ...
  const resetCreateFeeForm = () => {
    ...
    setCreateApplicableFromEnabled(false); // Reset toggle
  ...
  const resetEditFeeForm = () => {
    ...
    setEditingFee(null);
    setEditApplicableFromEnabled(false); // Reset toggle
    ...
    if (showAssignStudentsModal && selectedClassForAssignment && feeTypeToAssign && schoolId) {
      const fetchStudentsForClass = async () => {
        const { data, error } = await supabase
          .from('students')
          .select('id, name, roll_no') // Selecting only necessary fields
        ...
        if (error) {
          ...
          setStudentsInSelectedClass(data || []); // Data matches StudentForAssignment[] type
        }
      };
      ...
      const fetchExistingAssignments = async () => {
        const { data, error } = await supabase
          .from('student_fee_types')
          .select('id, student_id') // Fetching 'id' now as well for type consistency in state
        ...
        if (error) {
          ...
          setAssignedStudentFeeTypes(data || []); // Type is AssignedStudentFeeTypeRef[]
          setSelectedStudentIdsToAssign(data?.map(sft => sft.student_id) || []); // Pre-select existing
        }
      };
  ...
  const submitFeeType = async () => {
    ...
    if (createApplicableFromEnabled) { // Check toggle state
    ...
    const feeTypePayload: Database['public']['Tables']['fee_types']['Insert'] = {
      ...
      school_id: schoolId,
      applicable_from: applicableFromDate, // Use conditional date
  ...
  const openEditFeeModal = (feeType: FeeType) => {
    ...
    if (feeType.applicable_from) {
        applicableFromEnabled = true; // Enable toggle if date exists
    ...
    setEditApplicableFromEnabled(applicableFromEnabled); // Set toggle state
  ...
  const handleDuplicateFeeType = (feeType: FeeType) => {
    ...
    setCreateApplicableFromEnabled(applicableFromEnabled); // Set toggle state
  ...
  const updateFeeType = async () => {
    ...
    let applicableFromDate: string | null = null;
    if (editApplicableFromEnabled) { // Check toggle state
  ...
  const handleAssignStudentsToFee = async () => {
    if (!feeTypeToAssign || !schoolId || !selectedClassForAssignment) {
      toast.error("Invalid assignment data. Please select a fee type and class.");
      return;
    }
    ...
    try {
        ...
        const { data: currentAssignments, error: fetchAssignmentsError } = await supabase
            .from('student_fee_types')
            .select('id, student_id') // Fetching 'id' and 'student_id'
        ...
        const assignmentsToDeleteIds: number[] = []; // CORRECTED TYPE HERE to number[]
        ...
        selectedStudentIdsSet.forEach(studentId => {
            if (!currentAssignedStudentIds.has(studentId)) {
                assignmentsToAdd.push({
                    ...
                    discount: 0, // Default discount for new assignments
                    discount_description: null,
                    // REMOVED net_payable_amount as it's a generated column
                });
        ...
        currentAssignedStudentIds.forEach(studentId => {
            if (!selectedStudentIdsSet.has(studentId)) {
                ...
                if (assignmentToDelete) {
                    assignmentsToDeleteIds.push(assignmentToDelete.id); // 'id' is a number
                    studentsToRecalculateFees.add(studentId);
                }
            }
        });
        ...
        resetAssignStudentsModal(); // Close modal and reset state
    } catch (error: any) {
              ...
              {feeTypes.map((fee) => (
                  <tr key={fee.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-3 whitespace-nowrap text-sm font-medium space-x-1 text-center">
                      ...
                      <button onClick={() => { setFeeTypeToAssign(fee); setShowAssignStudentsModal(true); }} disabled={isPageLoading || isSubmittingFee} className="text-blue-600 hover:text-blue-800 p-1 disabled:text-gray-400" title="Assign to Students"> {/* NEW: Assign Students Button */}
              ...
              <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); modalProps.submit(); }} className="mt-5 space-y-5">
                    ...
                    {modalProps.applicableFromEnabled && ( // Conditionally render date pickers
                      <div>
                  ...
                  <div className="mt-5 space-y-6">
                    <div>
                      ...
                      >
                        ...
                        {filteredClassesForAssignStudents.map(cls => ( // CHANGED: Using filteredClassesForAssignStudents
